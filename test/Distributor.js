...
contract("Distributor", function (accounts) {
  ...
  describe("editRecipient", () => {
    before(async () => {
      distributor = await Distributor.new(tarot.address, claimable.address);
      claimable.setRecipient(distributor.address);
      await tarot.transfer(address(1), await tarot.balanceOf(recipientA), {
        from: recipientA,
      });
      await tarot.transfer(address(1), await tarot.balanceOf(recipientB), {
        from: recipientB,
      });
      await tarot.transfer(address(1), await tarot.balanceOf(recipientC), {
        from: recipientC,
      });
    });
    ...
    it("editRecipient updates shareIndex", async () => {
      ...
      expectEqual(await distributor.totalShares(), 10);
      const shareIndex = await distributor.shareIndex();
      const { shares, lastShareIndex } = await distributor.recipients(
        recipientB
      );
      expectEqual(shares, 5);
      expectEqual(shareIndex / 2 ** 160, 200);
    ...
    it("editRecipient calls updateCredit before editing shares", async () => {
      ...
      expectEqual(await distributor.totalShares(), 5);
      const shareIndex = await distributor.shareIndex();
      const { shares, lastShareIndex, credit } = await distributor.recipients(
        recipientA
      );
