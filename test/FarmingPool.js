...
contract("FarmingPool", function (accounts) {
  ...
  describe("advance onestep", () => {
    before(async () => {
      ...
      vester = await VesterOneStep.new(
        tarot.address,
        root,
        VESTING_AMOUNT,
        VESTING_BEGIN,
      ...
      farmingPool = await FarmingPool.new(
        tarot.address,
        vester.address,
        borrowable,
        vester.address
      ...
      await farmingPool.trackBorrow(borrowerA, oneMantissa, oneMantissa, {
        from: borrowable,
      });
    });
    ...
    it("after vestingBegin the first time", async () => {
      const updateTime = VESTING_BEGIN.add(
        VESTING_PERIOD.mul(new BN(1)).div(new BN(10))
      );
    ...
    it("following advance have no effect", async () => {
      const updateTime1 = VESTING_BEGIN.add(
        VESTING_PERIOD.mul(new BN(2)).div(new BN(10))
      );
      const updateTime2 = VESTING_BEGIN.add(
        VESTING_PERIOD.mul(new BN(11)).div(new BN(10))
      );
  ...
  describe("advance stepped", () => {
    ...
    before(async () => {
      tarot = await Tarot.new(root);
      vester = await VesterStepped.new(
        tarot.address,
        root,
        VESTING_AMOUNT,
        VESTING_BEGIN,
    ...
    it("starting after a few epochs", async () => {
      const updateTime = VESTING_BEGIN.add(
        VESTING_PERIOD.mul(new BN(25)).div(new BN(1000))
      );
      const epochBegin = VESTING_BEGIN.add(
        VESTING_PERIOD.mul(new BN(20)).div(new BN(1000))
      );
      await setTimestamp(updateTime);
      await farmingPool.advance();
    ...
    it("going to the next epoch", async () => {
      const updateTime = VESTING_BEGIN.add(
        VESTING_PERIOD.mul(new BN(37)).div(new BN(1000))
      );
      const epochBegin = VESTING_BEGIN.add(
        VESTING_PERIOD.mul(new BN(30)).div(new BN(1000))
      );
      await setTimestamp(updateTime);
      ...
      expectAlmostEqualMantissa(
        await farmingPool.shareIndex(),
        epochAmount1.mul(SHARE_INDEX_MULTIPLIER).div(SHARES_MULTIPLIER)
      );
    ...
    it("skipping epochs", async () => {
      const updateTime = VESTING_BEGIN.add(
        VESTING_PERIOD.mul(new BN(51)).div(new BN(1000))
      );
      const epochBegin = VESTING_BEGIN.add(
        VESTING_PERIOD.mul(new BN(50)).div(new BN(1000))
      );
      await setTimestamp(updateTime);
      await farmingPool.advance();
      expectEqual(await farmingPool.epochBegin(), epochBegin);
      ...
      expectAlmostEqualMantissa(
        ...
        epochAmount1
          .add(epochAmount2)
          .mul(SHARE_INDEX_MULTIPLIER)
          .div(SHARES_MULTIPLIER)
      );
      await farmingPool.claim({ from: borrowerA });
      expectAlmostEqualMantissa(
        await tarot.balanceOf(borrowerA),
        epochAmount1.add(epochAmount2).add(epochAmount3.div(new BN(10)))
      );
    });
  });
  ...
  describe("trackBorrow", () => {
    ...
    it("permissions check", async () => {
      await expectRevert(
        farmingPool.trackBorrow(borrowerA, oneMantissa, oneMantissa, {
          from: root,
        }),
        "FarmingPool: UNAUTHORIZED"
      );
    });

    it("increasing a user shares increases totalShare", async () => {
      await farmingPool.trackBorrow(
        borrowerA,
        oneMantissa.mul(new BN(16)),
        oneMantissa,
        { from: borrowable }
      );
      expectEqual(
        await farmingPool.totalShares(),
        SHARES_MULTIPLIER.mul(new BN(16))
      );
    ...
    it("decreasing a user shares decreases totalShare", async () => {
      ...
      expectEqual(
        await farmingPool.totalShares(),
        SHARES_MULTIPLIER.mul(new BN(8))
      );
      const { shares } = await farmingPool.recipients(borrowerA);
    ...
    it("trackBorrow updates shareIndex before updating shares", async () => {
      ...
      await farmingPool.trackBorrow(
        borrowerB,
        oneMantissa.mul(new BN(32)),
        oneMantissa.mul(new BN(4)),
        { from: borrowable }
      );
      expectEqual(await farmingPool.totalShares(), SHARES.mul(new BN(2)));
      const shareIndex = await farmingPool.shareIndex();
      const { shares, lastShareIndex } = await farmingPool.recipients(
        borrowerB
      );
      expectEqual(shares, SHARES);
      expectEqual(
        shareIndex,
        VESTING_AMOUNT.mul(SHARE_INDEX_MULTIPLIER).div(INV_TIME).div(SHARES)
      );
    ...
    it("trackBorrow set to 0 and calls updateCredit before editing shares", async () => {
      ...
      await setTimestamp(
        VESTING_BEGIN.add(VESTING_PERIOD.div(new BN(INV_TIME)))
      );
      await farmingPool.trackBorrow(
        borrowerA,
        "0",
        oneMantissa.mul(new BN(5)),
        { from: borrowable }
      );
      expectEqual(
        await farmingPool.totalShares(),
      ...
      expectEqual(
        credit,
        VESTING_AMOUNT.div(INV_TIME).mul(new BN(3)).div(new BN(4))
      );
    });
  });
  ...
  describe("scenario", () => {
    before(async () => {
      ...
      await tarot.transfer(vester.address, VESTING_AMOUNT);
      distributor = await OwnedDistributor.new(
        tarot.address,
        vester.address,
        admin
      );
      ...
      farmingPool2 = await FarmingPool.new(
        tarot.address,
        distributor.address,
        borrowable2,
        vester.address
      ...
      await distributor.editRecipient(farmingPool.address, "100", {
        from: admin,
      });
    });
    ...
    it("first epochs with only a farmingPool", async () => {
      ...
      await setTimestamp(
        VESTING_BEGIN.add(VESTING_PERIOD.mul(new BN(5)).div(new BN(1000)))
      );
      await farmingPool.claim({ from: borrowerA });
      await farmingPool.trackBorrow(
        borrowerB,
      ...
      await setTimestamp(
        VESTING_BEGIN.add(VESTING_PERIOD.mul(new BN(15)).div(new BN(1000)))
      );
      await farmingPool.claim({ from: borrowerA });
      await farmingPool.trackBorrow(
        borrowerC,
      ...
      await setTimestamp(
        VESTING_BEGIN.add(VESTING_PERIOD.mul(new BN(20)).div(new BN(1000)))
      );
      await farmingPool.claim({ from: borrowerB });
      await farmingPool.trackBorrow(borrowerB, 0, oneMantissa, {
        from: borrowable,
      });
      await setTimestamp(
        VESTING_BEGIN.add(VESTING_PERIOD.mul(new BN(25)).div(new BN(1000)))
      );
      await farmingPool.trackBorrow(
        borrowerA,
        oneMantissa.mul(new BN(4)),
      ...
      await setTimestamp(
        VESTING_BEGIN.add(VESTING_PERIOD.mul(new BN(30)).div(new BN(1000)))
      );
      await farmingPool.claim({ from: borrowerA });
      await farmingPool.claim({ from: borrowerC });
      expectAlmostEqualMantissa(
        await tarot.balanceOf(borrowerA),
        VESTING_AMOUNT.mul(new BN(39662)).div(new BN(1000000))
      );
      expectAlmostEqualMantissa(
        await tarot.balanceOf(borrowerB),
        VESTING_AMOUNT.mul(new BN(23757)).div(new BN(1000000))
      );
      expectAlmostEqualMantissa(
        await tarot.balanceOf(borrowerC),
        VESTING_AMOUNT.mul(new BN(13657)).div(new BN(1000000))
      );
    });

    it("adding a second farmingPool", async () => {
      await distributor.editRecipient(farmingPool2.address, "100", {
        from: admin,
      });
      await setTimestamp(
        VESTING_BEGIN.add(VESTING_PERIOD.mul(new BN(45)).div(new BN(1000))),
        [farmingPool2]
      );
      await farmingPool2.trackBorrow(borrowerB, oneMantissa, oneMantissa, {
        from: borrowable2,
      });
      await farmingPool2.claim({ from: borrowerB });
      await farmingPool.claim({ from: borrowerA });
      await distributor.editRecipient(farmingPool.address, "0", {
        from: admin,
      });
      await setTimestamp(
        VESTING_BEGIN.add(VESTING_PERIOD.mul(new BN(55)).div(new BN(1000))),
        [farmingPool2]
      );
      await farmingPool2.claim({ from: borrowerB });
      await farmingPool.claim({ from: borrowerA });
      ...
      expectAlmostEqualMantissa(
        await tarot.balanceOf(borrowerA),
        VESTING_AMOUNT.mul(new BN(63935)).div(new BN(1000000))
      );
      expectAlmostEqualMantissa(
        await tarot.balanceOf(borrowerB),
        VESTING_AMOUNT.mul(new BN(47352)).div(new BN(1000000))
      );
      expectAlmostEqualMantissa(
        await tarot.balanceOf(borrowerC),
        VESTING_AMOUNT.mul(new BN(25794)).div(new BN(1000000))
      );
    });
  });
});
