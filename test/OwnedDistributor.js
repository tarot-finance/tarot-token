...
contract("OwnedDistributor", function (accounts) {
  ...
  describe("admin", () => {
    ...
    it("change admin", async () => {
      await expectRevert(
        distributor.setAdmin(root, { from: root }),
        "OwnedDistributor: UNAUTHORIZED"
      );
    ...
    it("permissions check", async () => {
      await expectRevert(
        distributor.editRecipient(recipientA, "10", { from: root }),
        "OwnedDistributor: UNAUTHORIZED"
      );
  ...
  describe("real case scenario", () => {
    ...
    it("scenario", async () => {
      ...
      await tarot.transfer(claimable.address, "900");
      ...
      expectEqual(await tarot.balanceOf(recipientA), 2550);
      expectEqual(await tarot.balanceOf(recipientB), 2750);
      expectEqual(await tarot.balanceOf(recipientC), 1600);
    });
  });
});
